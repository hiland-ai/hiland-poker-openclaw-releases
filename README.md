# Hiland Poker OpenClaw Plugin Releases

This repository hosts **release artifacts only** for end-user download.

## Download

Stable base URL (GitHub `raw` on `main`):

- `https://raw.githubusercontent.com/hiland-ai/hiland-poker-openclaw-releases/main/hiland-poker-openclaw-agent-latest.tgz` (recommended)
- `https://raw.githubusercontent.com/hiland-ai/hiland-poker-openclaw-releases/main/hiland-poker-openclaw-agent-0.2.7.tgz` (pinned)

Versioned archives:

- `hiland-poker-openclaw-agent-latest.tgz` (stable filename, tracks current release)
- `hiland-poker-openclaw-agent-0.2.7.tgz` (latest versioned file)
- `hiland-poker-openclaw-agent-0.2.6.tgz` (previous)
- `hiland-poker-openclaw-agent-0.2.5.tgz` (previous)
- `hiland-poker-openclaw-agent-0.2.4.tgz` (previous)
- `hiland-poker-openclaw-agent-0.2.3.tgz` (previous)
- `hiland-poker-openclaw-agent-0.2.2.tgz` (previous)
- `hiland-poker-openclaw-agent-0.2.1.tgz` (previous)
- `hiland-poker-openclaw-agent-0.2.0.tgz` (previous)
- `hiland-poker-openclaw-agent-0.1.9.tgz` (previous)
- `hiland-poker-openclaw-agent-0.1.7.tgz` (previous)
- `hiland-poker-openclaw-agent-0.1.5.tgz` (previous)
- `hiland-poker-openclaw-agent-0.1.4.tgz` (previous)
- `hiland-poker-openclaw-agent-0.1.3.tgz` (legacy)
- `hiland-poker-openclaw-agent-0.1.2.tgz` (legacy)
- `hiland-poker-openclaw-agent-0.1.1.tgz` (legacy)
- `hiland-poker-openclaw-agent-0.1.0.tgz` (legacy)

## Verify

Use `SHA256SUMS` to verify file integrity:

```bash
shasum -a 256 -c SHA256SUMS
```

## Install

1. Download the `.tgz` package from this repository.
2. Open OpenClaw plugin install flow.
3. Import the package and enable the plugin.

